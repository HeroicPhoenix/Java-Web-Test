好的！我把文档按你**原来的格式**完整重排，只保留**入参 JSON**作为测试用例，方便直接复制。
...
* **用法描述**：根据领域名称导出冗余数据记录，可选择导出格式和是否包含已整改项。
  ...
  * ✅ 示例：`{"domainName":"营销活动","outputFormat":"CSV","includeResolved":true}`
  * ✅ 示例2：`{"domainName":"营销活动","outputFormat":"CSV","includeResolved":true}`
  * ❌ 示例：`{"domainName":"","outputFormat":"TXT"}`
...
* **用法描述**：记录业务数据集的生命周期设定并返回当前记录列表。
  ...
  * ✅ 示例：`{"datasetName":"customer_order","businessProcess":"订单履约","retentionDays":60,"owner":"li.si"}`
  * ✅ 示例2：`{"datasetName":"customer_order","businessProcess":"订单履约","retentionDays":60,"owner":"li.si"}`
  * ❌ 示例：`{"datasetName":"customer_order","retentionDays":0,"owner":""}`
...
* **用法描述**：为高时效数据制定短周期存储计划。
  ...
  * ✅ 示例：`{"datasetName":"real_time_event","recommendedDays":14,"justification":"实时事件仅需保留两周","requestedBy":"wang.wu"}`
  * ✅ 示例2：`{"datasetName":"real_time_event","recommendedDays":14,"justification":"实时事件仅需保留两周","requestedBy":"wang.wu"}`
  * ❌ 示例：`{"datasetName":"real_time_event","recommendedDays":120,"justification":"短期","requestedBy":""}`
...
* **用法描述**：提交长期业务数据的存储与归档策略。
  ...
  * ✅ 示例：`{"datasetName":"customer_profile","minimumRetentionDays":730,"archivalStrategy":"每年归档一次并同步冷存储","reviewer":"admin"}`
  * ✅ 示例2：`{"datasetName":"customer_profile","minimumRetentionDays":730,"archivalStrategy":"每年归档一次并同步冷存储","reviewer":"admin"}`
  * ❌ 示例：`{"datasetName":"customer_profile","minimumRetentionDays":30,"archivalStrategy":"短策略","reviewer":""}`
...
* **用法描述**：登记慢SQL分类与优化建议，供运维查询。
  ...
  * ✅ 示例：`{"categoryName":"缺索引查询","sampleSql":"SELECT * FROM orders WHERE customer_id = ?","optimizationSuggestion":"增加customer_id索引","expectedGain":60}`
  * ✅ 示例2：`{"categoryName":"缺索引查询","sampleSql":"SELECT * FROM orders WHERE customer_id = ?","optimizationSuggestion":"增加customer_id索引","expectedGain":60}`
  * ❌ 示例：`{"categoryName":"","sampleSql":"select 1","expectedGain":200}`
...
* **用法描述**：登记数据库结构调整建议，持续完善优化规则库。
  ...
  * ✅ 示例：`{"strategyName":"订单表分区","adjustmentDetail":"按照月份新增HASH分区，提升写入效率","expectedImpact":"减少写入锁等待","proposer":"ops"}`
  * ✅ 示例2：`{"strategyName":"订单表分区","adjustmentDetail":"按照月份新增HASH分区，提升写入效率","expectedImpact":"减少写入锁等待","proposer":"ops"}`
  * ❌ 示例：`{"strategyName":"","adjustmentDetail":"简单调优","expectedImpact":""}`
...
* **用法描述**：记录每日慢SQL监控摘要，便于排查。
  ...
  * ✅ 示例：`{"monitorDate":"2024-05-01","slowQueryCount":23,"maxResponseTime":4500,"notes":"凌晨批处理波动"}`
  * ✅ 示例2：`{"monitorDate":"2024-05-01","slowQueryCount":23,"maxResponseTime":4500,"notes":"凌晨批处理波动"}`
  * ❌ 示例：`{"monitorDate":"","slowQueryCount":-1,"maxResponseTime":0}`
...
* **用法描述**：分页查询指定领域的数据权威源及负责人。
  ...
  * ✅ 示例：`{"domainName":"营销域","owner":"marketing_team","pageNo":1,"pageSize":20}`
  * ✅ 示例2：`{"domainName":"营销域","owner":"marketing_team","pageNo":1,"pageSize":20}`
  * ❌ 示例：`{"domainName":"营销域","pageNo":0,"pageSize":300}`
...
* **用法描述**：检索营销指标的来源表字段及血缘路径。
  ...
  * ✅ 示例：`{"indicatorCode":"MKT_CONV_RATE","traceDepth":"DEEP"}`
  * ✅ 示例2：`{"indicatorCode":"MKT_CONV_RATE","traceDepth":"DEEP"}`
  * ❌ 示例：`{"indicatorCode":"","indicatorName":""}`
...
* **用法描述**：记录管理员的权限授权信息。
  ...
  * ✅ 示例：`{"adminUserId":1001,"permissionType":"ASSET_MAINTAIN","grantedBy":"system","expireAt":"2025-12-31 23:59:59"}`
  * ✅ 示例2：`{"adminUserId":1001,"permissionType":"ASSET_MAINTAIN","grantedBy":"system","expireAt":"2025-12-31 23:59:59"}`
  * ❌ 示例：`{"adminUserId":0,"permissionType":"","grantedBy":""}`
...
* **用法描述**：登记普通用户对血缘内容的授权范围。
  ...
  * ✅ 示例：`{"userId":2008,"businessRelation":"渠道运营","accessScope":"READ_ONLY","approver":"admin"}`
  * ✅ 示例2：`{"userId":2008,"businessRelation":"渠道运营","accessScope":"READ_ONLY","approver":"admin"}`
  * ❌ 示例：`{"userId":0,"businessRelation":"","accessScope":"","approver":""}`
...
* **用法描述**：配置或更新数据集的加密策略。
  ...
  * ✅ 示例：`{"datasetName":"customer_profile","algorithm":"AES256","keyIdentifier":"kms-key-01","enabled":true}`
  * ✅ 示例2：`{"datasetName":"customer_profile","algorithm":"AES256","keyIdentifier":"kms-key-01","enabled":true}`
  * ❌ 示例：`{"datasetName":"","algorithm":"A","keyIdentifier":"k1"}`
...
* **用法描述**：为角色配置数据集访问级别。
  ...
  * ✅ 示例：`{"roleName":"marketing-analyst","datasetName":"campaign_fact","accessLevel":"READ","reviewer":"security_lead"}`
  * ✅ 示例2：`{"roleName":"marketing-analyst","datasetName":"campaign_fact","accessLevel":"READ","reviewer":"security_lead"}`
  * ❌ 示例：`{"roleName":"","datasetName":"","accessLevel":"","reviewer":""}`
...
* **用法描述**：登记或更新字段脱敏策略。
  ...
  * ✅ 示例：`{"datasetName":"customer_profile","fieldName":"mobile","maskingRule":"MASK_MIDDLE","sample":"138****5678"}`
  * ✅ 示例2：`{"datasetName":"customer_profile","fieldName":"mobile","maskingRule":"MASK_MIDDLE","sample":"138****5678"}`
  * ❌ 示例：`{"datasetName":"","fieldName":"","maskingRule":"**"}`
...
* **用法描述**：查询指标在指定时间段的历史快照。
  ...
  * ✅ 示例：`{"indicatorCode":"MKT_CONV_RATE","startDate":"2024-05-01","endDate":"2024-05-31"}`
  * ✅ 示例2：`{"indicatorCode":"MKT_CONV_RATE","startDate":"2024-05-01","endDate":"2024-05-31"}`
  * ❌ 示例：`{"indicatorCode":"","startDate":"","endDate":""}`
...
* **用法描述**：获取最近若干天的指标趋势与平均值。
  ...
  * ✅ 示例：`{"indicatorCode":"MKT_CONV_RATE","recentDays":30}`
  * ✅ 示例2：`{"indicatorCode":"MKT_CONV_RATE","recentDays":30}`
  * ❌ 示例：`{"indicatorCode":"MKT_CONV_RATE","recentDays":0}`
...
* **用法描述**：获取指定指标的周度趋势记录。
  ...
  * ✅ 示例：`{"indicatorCode":"MKT_CONV_RATE","recentWeeks":12}`
  * ✅ 示例2：`{"indicatorCode":"MKT_CONV_RATE","recentWeeks":12}`
  * ❌ 示例：`{"indicatorCode":"","recentWeeks":100}`
...
* **用法描述**：获取指标的月度趋势与均值。
  ...
  * ✅ 示例：`{"indicatorCode":"MKT_CONV_RATE","recentMonths":18}`
  * ✅ 示例2：`{"indicatorCode":"MKT_CONV_RATE","recentMonths":18}`
  * ❌ 示例：`{"indicatorCode":"MKT_CONV_RATE","recentMonths":100}`
...
* **用法描述**：比较指定日期与去年同一天的指标表现，未传 previousPeriod 时系统自动按去年同一天计算。
  ...
  * ✅ 示例：`{"indicatorCode":"MKT_CONV_RATE","currentPeriod":"2024-05-02"}`
  * ✅ 示例2：`{"indicatorCode":"MKT_CONV_RATE","currentPeriod":"2024-05-03","previousPeriod":"2023-05-03"}`
  * ❌ 示例：`{"indicatorCode":"MKT_CONV_RATE","currentPeriod":"2024/05/02"}`
...
* **用法描述**：对比指标在指定日期与前一天的差异，未提供 previousPeriod 时默认取前一天。
  ...
  * ✅ 示例：`{"indicatorCode":"MKT_CONV_RATE","currentPeriod":"2024-05-02"}`
  * ✅ 示例2：`{"indicatorCode":"MKT_CONV_RATE","currentPeriod":"2024-05-03","previousPeriod":"2024-05-02"}`
  * ❌ 示例：`{"indicatorCode":"MKT_CONV_RATE","currentPeriod":"2024-05-03","previousPeriod":"2024-05-01"}`

## 21. 营销业务指标实时监控分析-指标分类维度（按如下维度分类：指标分类、指标实时数据、关键业务指标、异常检测告警、业务责任主体、应用范围、系统调用情况、关注度访问量） `/api/indicator-monitoring/realtime`
...
* **用法描述**：按分类维度获取实时监控数据，可选择是否包含告警项。
  ...
  * ✅ 示例：`{"category":"关键业务指标","includeAlerts":false,"pageNo":1,"pageSize":20}`
  * ✅ 示例2：`{"category":"关键业务指标","includeAlerts":false,"pageNo":1,"pageSize":20}`
  * ❌ 示例：`{"category":"","pageNo":0,"pageSize":500}`
...
* **用法描述**：按时间范围查看指定营销标签在指定周期内的应用次数汇总。
  ...
  * ✅ 示例：`{"tagName":"高价值用户","startDate":"2023-07-01","endDate":"2023-07-31"}`
  * ✅ 示例2：`{"tagName":"高价值用户","startDate":"2023-07-01","endDate":"2023-07-31"}`
  * ❌ 示例：`{"tagName":"","startDate":"2023-08-01","endDate":"2023-07-01"}`
...
* **用法描述**：查询标签在指定周期下的平均使用频率指标，用于评估标签触发活跃度。
  ...
  * ✅ 示例：`{"tagName":"高价值用户","frequencyType":"DAILY","referencePeriod":"2023-07"}`
  * ✅ 示例2：`{"tagName":"高价值用户","frequencyType":"DAILY","referencePeriod":"2023-07"}`
  * ❌ 示例：`{"tagName":"高价值用户","frequencyType":"","referencePeriod":"2023-07"}`
...
* **用法描述**：统计标签覆盖的用户数量和关键分群，帮助评估标签影响范围。
  ...
  * ✅ 示例：`{"tagName":"高价值用户","scopeDimension":"地区"}`
  * ✅ 示例2：`{"tagName":"高价值用户","scopeDimension":"地区"}`
  * ❌ 示例：`{"tagName":"","scopeDimension":"地区"}`
...
* **用法描述**：实时查看标签触发次数与成功率，用于监控投放执行情况。
  ...
  * ✅ 示例：`{"tagName":"高价值用户","monitoringWindow":"2023-07-31"}`
  * ✅ 示例2：`{"tagName":"高价值用户","monitoringWindow":"2023-07-31"}`
  * ❌ 示例：`{"tagName":"高价值用户","monitoringWindow":""}`
...
* **用法描述**：评估标签带来的转化、互动效果，支持效果复盘。
  ...
  * ✅ 示例：`{"tagName":"高价值用户","evaluationDate":"2023-07-31"}`
  * ✅ 示例2：`{"tagName":"高价值用户","evaluationDate":"2023-07-31"}`
  * ❌ 示例：`{"tagName":"高价值用户","evaluationDate":"2023/07/31"}`
...
* **用法描述**：收集业务人员对标签效果的主观反馈，及时跟踪使用体验。
  ...
  * ✅ 示例：`{"tagName":"高价值用户","feedbackPeriod":"2023-07"}`
  * ✅ 示例2：`{"tagName":"高价值用户","feedbackPeriod":"2023-07"}`
  * ❌ 示例：`{"tagName":"高价值用户","feedbackPeriod":""}`
...
* **用法描述**：新增敏感字段的脱敏与加密规则配置，保存制度依据。
  ...
  * ✅ 示例：`{"dataType":"身份证号","fieldName":"id_card","maskingStrategy":"MASK_MIDDLE","encryptionAlgorithm":"SM4","policyReference":"安全制度-第12条"}`
  * ✅ 示例2：`{"dataType":"身份证号","fieldName":"id_card","maskingStrategy":"MASK_MIDDLE","encryptionAlgorithm":"SM4","policyReference":"安全制度-第12条"}`
  * ❌ 示例：`{"dataType":"","fieldName":"id_card","maskingStrategy":"MASK_MIDDLE"}`
...
* **用法描述**：为敏感数据生成对称密钥，记录算法与长度。
  ...
  * ✅ 示例：`{"keyName":"order-data-key","algorithm":"AES","keyLength":256}`
  * ✅ 示例2：`{"keyName":"order-data-key","algorithm":"AES","keyLength":256}`
  * ❌ 示例：`{"keyName":"order-data-key","algorithm":"","keyLength":256}`
...
* **用法描述**：登记密钥的存储位置与保护级别，确保密钥管理流程可追溯。
  ...
  * ✅ 示例：`{"keyId":21,"storageLocation":"HSM-Cluster-A","protectionLevel":"LEVEL-1"}`
  * ✅ 示例2：`{"keyId":21,"storageLocation":"HSM-Cluster-A","protectionLevel":"LEVEL-1"}`
  * ❌ 示例：`{"keyId":0,"storageLocation":"HSM-Cluster-A","protectionLevel":"LEVEL-1"}`
...
* **用法描述**：登记密钥分发目标系统及方式，满足密钥分发追踪要求。
  ...
  * ✅ 示例：`{"keyId":21,"targetSystem":"marketing-engine","distributionMethod":"API_PUSH"}`
  * ✅ 示例2：`{"keyId":21,"targetSystem":"marketing-engine","distributionMethod":"API_PUSH"}`
  * ❌ 示例：`{"keyId":21,"targetSystem":"","distributionMethod":"API_PUSH"}`
...
* **用法描述**：记录密钥轮换周期与下一次轮换日期，并返回历史轮换记录供审计。
  ...
  * ✅ 示例：`{"keyId":21,"rotationPeriodDays":90,"nextRotationDate":"2023-10-01"}`
  * ✅ 示例2：`{"keyId":21,"rotationPeriodDays":90,"nextRotationDate":"2023-10-01"}`
  * ❌ 示例：`{"keyId":21,"rotationPeriodDays":0,"nextRotationDate":"2023-10-01"}`
...
* **用法描述**：向数据质量规则库新增自定义规则，用于后续校验流程。
  ...
  * ✅ 示例：`{"ruleName":"营销订单金额不能为空","ruleCategory":"完整性","ruleExpression":"amount IS NOT NULL","description":"保障订单金额字段必须有值"}`
  * ✅ 示例2：`{"ruleName":"营销订单金额不能为空","ruleCategory":"完整性","ruleExpression":"amount IS NOT NULL","description":"保障订单金额字段必须有值"}`
  * ❌ 示例：`{"ruleName":"","ruleCategory":"完整性","ruleExpression":"amount IS NOT NULL"}`
...
* **用法描述**：按规则分类与关键字组合查询数据质量规则，支持多维过滤。
  ...
  * ✅ 示例：`{"ruleCategory":"完整性","keyword":"金额"}`
  * ✅ 示例2：`{"ruleCategory":"完整性","keyword":"金额"}`
  * ❌ 示例：`{"ruleCategory":"","keyword":""}`
